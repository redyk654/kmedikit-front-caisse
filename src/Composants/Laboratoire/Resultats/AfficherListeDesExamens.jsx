...
export default function AfficherListeDesExamens({ afficherExamens, valeurRecherche, vueListeDesExamens }) {
  // const vueListeDesExamens = filtrerListe('nom', valeurRecherche, listeDesExamens);
  if(valeurRecherche !== '' && vueListeDesExamens.length === 0) return <p className='text-center h2'>
    Aucun résultat pour <span className='text-primary fw-bold'>{valeurRecherche}</span>
  </p>

  if(vueListeDesExamens.length === 0) return <p className='text-center h2'>Chargement...</p>
